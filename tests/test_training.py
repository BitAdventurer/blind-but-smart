...
def config():
    return {
        ...
        "reference_controller": {"hidden_sizes": [16, 16]},
        "training": {"replay_batch_size": 8,
                     "fixed_complement": {"regional_budget": 3.0, "candidate_count": 5}},
    }
...
class TrainingTests(unittest.TestCase):
    ...
    def tensor_batch(self, arrays=None):
        return {key: torch.from_numpy(value) for key, value in (fixture_arrays() if arrays is None else arrays).items()}
    ...
    def test_missing_fixed_schedule_is_rejected(self):
        with self.assertRaisesRegex(ValueError, "fixed_complement"):
    ...
    def test_single_head_variants_update(self):
        for method in ("Disclosure-only", "Count-only", "Independent-1M"):
            with self.subTest(method=method):
                ...
                result = trainer.evaluation_action(np.zeros((2, 28), np.float32), np.full(2, 100.0))
    ...
    def test_checkpoint_rejects_different_data_method_or_configuration(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            altered_config["reference_controller"]["optimizer"] = {"lr": 0.0001}
            with self.assertRaisesRegex(ValueError, "config_sha256"):
                Trainer(altered_config, "H", trainer.buffer).load_checkpoint(path)
