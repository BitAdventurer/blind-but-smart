...
class Trainer:
    ...
    def __init__(self, config: dict, method: str, buffer: ReplayBuffer,
                 ...
                 tms_schedule: TMSSchedule | None = None, task: str | None = None,
        ...
        # Capture the post-initialization state inside the seeded context; the
        # run owns future proposal draws without consuming a caller's stream.
        ...
        self.tms_schedule = tms_schedule
        self.task, self.family = task, family
        if "fixed_complement" in self.training:
    ...
    def _validate_schedule(self, schedule):
        if not isinstance(schedule, TMSSchedule):
            raise TypeError("tms_schedule must be a validated TMSSchedule")
        if schedule.task != self.task or schedule.family != self.family:
            raise ValueError("TMS task/family does not match the explicitly bound run")
    ...
    def _action(self, bundle: dict, observation: torch.Tensor, remaining: torch.Tensor,
                *, update_index: int, deterministic: bool = False, slot_ids=None,
                tms_schedule=None) -> dict:
        actor = bundle["actor"]
        sample = actor.evaluation_action(observation) if deterministic else actor.sample(observation, update_index=update_index)
        if actor.role != "joint":
            schedule = self.tms_schedule if tms_schedule is None else tms_schedule
            self._validate_schedule(schedule)
            budgets, counts = schedule.proposals(slot_ids, observation[:, 27].detach().cpu().numpy())
        if "budgets" not in sample:
            sample["budgets"] = torch.as_tensor(budgets, dtype=torch.float64, device=self.device)
        sample["executed_budgets"] = filter_budgets(sample["budgets"], remaining)
        if "candidate_count" not in sample:
            sample["candidate_count"] = torch.as_tensor(counts, dtype=torch.long, device=self.device)
    ...
    def _critic_input(observation: torch.Tensor, budgets: torch.Tensor, count_one_hot: torch.Tensor) -> torch.Tensor:
        return torch.cat((observation.to(torch.float32), budgets.to(torch.float32), count_one_hot.to(torch.float32)), dim=-1)
    ...
    def _target(self, bundle: dict, batch: dict, update_index: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            slot_ids = batch.get("next_slot_id")
            if slot_ids is not None:
                slot_ids = slot_ids[nonterminal.cpu().numpy()]
            action = self._action(bundle, next_observation, batch["next_remaining_budget"][nonterminal], update_index=update_index, slot_ids=slot_ids)
            ...
            if "categorical_log_prob" in action:
                # The target uses log pi of its independently sampled hard k;
                # exact categorical entropy is the actor-loss convention.
                value = value - 0.2 * action["categorical_log_prob"]
    ...
    def _update_component(self, name: str, batch: dict) -> dict:
        ...
        self.component_updates[name] = update_index + 1
        return {
    ...
    def step(self, batch_size: int | None = None) -> dict:
        ...
        with self._random_context():
            for name in self.bundles:
                ...
                batch = {key: value if key in ("slot_id", "next_slot_id") else torch.from_numpy(value).to(self.device)
                         for key, value in sampled.items() if key != "indices"}
    ...
    def evaluation_action(self, observation: np.ndarray | torch.Tensor,
                          remaining_budget: np.ndarray | torch.Tensor, *, slot_ids=None,
                          tms_schedule: TMSSchedule | None = None) -> dict[str, torch.Tensor]:
        """Compose separately fitted heads only here, using deterministic means."""
        action = self.proposal_action(observation, slot_ids=slot_ids, tms_schedule=tms_schedule)
        remaining = torch.as_tensor(remaining_budget, dtype=torch.float64, device=self.device)
        return {"executed_budgets": filter_budgets(action["budgets"], remaining), "candidate_count": action["candidate_count"]}

    def proposal_action(self, observation: np.ndarray | torch.Tensor, *, slot_ids=None,
                        tms_schedule: TMSSchedule | None = None) -> dict[str, torch.Tensor]:
        """Deterministic, unfiltered proposals; deployed ledger owns filtering.

        Single-head evaluation requires the evaluation population's bound TMS
        schedule, which may differ from the training population schedule.
        Independent composes its learned heads and does not query TMS at test.
        """
        with torch.no_grad():
            ...
            if self.method in ("Independent", "Independent-1M"):
                ...
                return {"budgets": budget, "candidate_count": count}
            bundle = next(iter(self.bundles.values()))
            action = bundle["actor"].evaluation_action(observation)
            if bundle["actor"].role != "joint":
                schedule = self.tms_schedule if tms_schedule is None else tms_schedule
                self._validate_schedule(schedule)
                budgets, counts = schedule.proposals(slot_ids, observation[:, 27].cpu().numpy())
                action.setdefault("budgets", torch.as_tensor(budgets, dtype=torch.float64, device=self.device))
                action.setdefault("candidate_count", torch.as_tensor(counts, dtype=torch.long, device=self.device))
            return action
    ...
    def save_checkpoint(self, path: str | Path) -> None:
        ...
        payload = {
            "format_version": 2, "status": "new_reference_not_original_runtime",
            "task": self.task, "family": self.family,
            "tms_schedule_sha256": self.tms_schedule.content_sha256 if self.tms_schedule else None,
